...
public class BusRouteOverlay extends RouteOverlay {
	...
	public BusRouteOverlay(Context context, AMap amap, BusPath path,
		...
		super(context);
		this.busPath = path;
		startPoint = MapUtils.latLonPointToLatLng(start);
		endPoint = MapUtils.latLonPointToLatLng(end);
		mAMap = amap;
	}
		...
		try {
			...
			for (int i = 0; i < busSteps.size(); i++) {
				...
				if (busStep.getBusLine() != null) {
					...
					if (i == busSteps.size() - 1) {
						addWalkPolyline(MapUtils.latLonPointToLatLng(getLastBuslinePoint(busStep)), endPoint);
					}
				}
				if (busStep.getRailway() != null) {
					...
					if (i == busSteps.size() - 1) {
						addWalkPolyline(MapUtils.latLonPointToLatLng(busStep.getRailway().getArrivalstop().getLocation()), endPoint);
					}
				}
	...
	private void addRailwayStep(RouteRailwayItem railway) {
		...
		for (int i = 0; i < railwayStationItems.size(); i++) {
			railwaylistpoint.add(MapUtils.latLonPointToLatLng(railwayStationItems.get(i).getLocation()));
		}
	...
	private void addTaxiStep(TaxiItem taxi){
		addPolyLine(new PolylineOptions().width(getRouteWidth())
				.color(getBusColor())
				.add(MapUtils.latLonPointToLatLng(taxi.getOrigin()))
				.add(MapUtils.latLonPointToLatLng(taxi.getDestination())));
	}
	...
	private void addWalkSteps(BusStep busStep) {
		...
		for (int j = 0; j < walkSteps.size(); j++) {
			...
			if (j == 0) {
				LatLng latLng = MapUtils.latLonPointToLatLng(walkStep
						.getPolyline().get(0));
			...
			List<LatLng> listWalkPolyline = MapUtils.latLonPointsToLatLngList(walkStep.getPolyline());
			...
			if (j < walkSteps.size() - 1) {
				LatLng lastLatLng = listWalkPolyline.get(listWalkPolyline
						.size() - 1);
				LatLng firstlatLatLng = MapUtils.latLonPointToLatLng(walkSteps.get(j + 1).getPolyline().get(0));
	...
	private void addBusLineSteps(List<LatLonPoint> listPoints) {
		...
		addPolyLine(new PolylineOptions().width(getRouteWidth())
				.color(getBusColor())
				.addAll(MapUtils.latLonPointsToLatLngList(listPoints)));
	}
	...
	private void addBusStationMarkers(RouteBusLineItem routeBusLineItem) {
		...
		LatLng position = MapUtils.latLonPointToLatLng(startBusStation
				.getLatLonPoint());
	...
	private void addTaxiMarkers(TaxiItem taxiItem) {
		...
		LatLng position = MapUtils.latLonPointToLatLng(taxiItem
				.getOrigin());
	...
	private void addRailwayMarkers(RouteRailwayItem railway) {
		LatLng Departureposition = MapUtils.latLonPointToLatLng(railway
		...
		LatLng Arrivalposition = MapUtils.latLonPointToLatLng(railway
	...
	private void checkBusToNextBusNoWalk(BusStep busStep, BusStep busStep1) {
		LatLng endbusLatLng = MapUtils.latLonPointToLatLng(getLastBuslinePoint(busStep));
		LatLng startbusLatLng = MapUtils.latLonPointToLatLng(getFirstBuslinePoint(busStep1));
	...
	private void checkBusEndToNextBusStart(BusStep busStep, BusStep busStep1) {
		LatLonPoint busLastPoint = getLastBuslinePoint(busStep);
		LatLng endbusLatLng = MapUtils.latLonPointToLatLng(busLastPoint);
		LatLonPoint busFirstPoint = getFirstBuslinePoint(busStep1);
		LatLng startbusLatLng = MapUtils.latLonPointToLatLng(busFirstPoint);
	...
	private void addWalkPolyLineByLatLonPoints(LatLonPoint pointFrom,
                                               LatLonPoint pointTo) {
		LatLng latLngFrom = MapUtils.latLonPointToLatLng(pointFrom);
		LatLng latLngTo = MapUtils.latLonPointToLatLng(pointTo);
