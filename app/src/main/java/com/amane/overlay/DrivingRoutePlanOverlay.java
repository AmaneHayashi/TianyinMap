...
public class DrivingRoutePlanOverlay extends RouteOverlay{
    ...
    public DrivingRoutePlanOverlay(Context context, AMap amap, DriveRoutePlanResult result, int index) {
        ...
        if (result != null) {
            startPoint          = MapUtils.latLonPointToLatLng(result.getStartPos());
            endPoint            = MapUtils.latLonPointToLatLng(result.getTargetPos());
        }
    }
    ...
    private void colorWayUpdate(List<TMC> tmcSection) {
        ...
        mPolylineOptionscolor.add(MapUtils.latLonPointToLatLng(tmcSection.get(0).getPolyline().get(0)));
        ...
        for (int i = 0; i < tmcSection.size(); i++) {
            ...
            for (int j = 1; j < mployline.size(); j++) {
                mPolylineOptionscolor.add(MapUtils.latLonPointToLatLng(mployline.get(j)));
                colorList.add(color);
            }
        }
        colorList.add(getDriveColor());
